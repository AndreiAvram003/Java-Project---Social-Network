...
public class FriendshipMapperImpl implements FriendshipMapper {
    @Override
    public Friendship toFriendship(User firstUser, User secondUser,String status) {
        return new Friendship(firstUser.getId(), secondUser.getId(), LocalDateTime.now(),status);
    }
